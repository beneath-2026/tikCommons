...
class RenderPalDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None, initial_data=None):
        ...
        self.job_name = QtWidgets.QLineEdit(initial_data.get("job_name", "Blender Job"))
...
class RenderPalSubmission(ExtensionCore):
    ...
    def generateOutDir(self):
        ...
        if not sequenceName or not shotName or not taskName:
            print("[RENDER ERROR] sequenceName or shotName or taskName missing")
            print(f"{sequenceName}, {shotName}, {taskName}")
            return None
    ...
    def start(self):
        ...
        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            ...
            bpy.data.scenes["Scene"].render.image_settings.media_type = "MULTI_LAYER_IMAGE"
            ...
            job_name_full = f"{settings.get('job_name')}: {os.path.basename(blend_path)}"
            ...
            try:
                ...
                LOG.error(f"Executable not found: {executable_path}")
                raise
            except Exception as e:
                LOG.error(f"Error submitting job: {str(e)}")
                raise
